...
# Pending FIRM January 26 # -----------------------------------------------------

# sfha area from 2026 pending FIRM downloaded january 7 2026
st_clip_sfha_2026 <- st_read("inputs/Cook_2026_download/S_FLD_HAZ_AR.shp") |>
  filter(SFHA_TF == "T") |>
  st_transform("EPSG:6454") |>
  st_intersection(border)

fld_haz_ar_2026 <- st_cast(st_clip_sfha_2026, "MULTIPOLYGON")
# fld_haz_ar_2026 <- st_transform(fld_haz_ar_2026, common_crs)


## 2026 SFHA parcels
tic()
beep_on_error(parcels_sfha_2026 <- st_join(parcels_2024, fld_haz_ar_2026, join = st_intersects), sound = "wilhelm") # kept all 1.44 million parcels_2024
parcels_sfha_2026 <- parcels_sfha_2026 |> filter(!is.na(DFIRM_ID))
write_sf(parcels_sfha_2026, "./data/processed/parcels_sfha_2026.shp")
beep("coin")
toc()


# and as a CSV with less variables
parcels_sfha_2026 <- sf::read_sf("data/processed/parcels_sfha_2026.shp")

sfha_2026 <- parcels_sfha_2026 |> as.data.frame() |>
  # select(-c(GFID:shape)) |>
  select(pin = name,
    DFIRM_ID = DFIRM_I, FLD_ZON = FLD_ZON, ZONE_SUBTY = ZONE_SU, FLD_AR_ = FLD_AR_) |>
  group_by(pin) |>
  slice(1) |>
  ungroup()

sfha_2026 |>  write_csv("data/processed/sfha_pins_2026.csv")
...
sfha2026 <- read_csv("data/processed/sfha_pins_2026.csv") |>
  mutate(pin10 = str_sub(pin, 1, 10)) |>
  select(pin10, FLD_ZONE = FLD_ZON, FLD_AR_, ZONE_SUBTY) |>
  distinct()
...
pin_indicators <- sfha2024 |> full_join(sfha2026, by = "pin10", suffix = c("2024", "prelim"))
