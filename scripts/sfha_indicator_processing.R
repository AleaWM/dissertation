...
# The code assumes the eight Excel files live in your working
...
sfha_compare_pin10_fourway |>
  ...
  filter(!pin10 %in% indicator_combined$pin10 & risk500_bldg_2024 == 1)

sfha_compare_pin10_fourway |>
  filter(!pin10 %in% drop_parcels) |>
  filter(!pin10 %in% indicator_combined$pin10 & land_sfha2024 == 1)

# goes through make_sfha_indicator_final() function in its own R file
# BEFORE becoming the sfha_compare_pin10_fourway file in the _targets.R workflow.
...
parcels_with_firm_dates <- left_join(parcels_with_firms, firm_dates_file, by = "FIRM_PAN") |> distinct()

# check if parcels mapped to more than one FIRM panel
#     currently failing this test lol
#     stopifnot(parcels_with_firms |> group_by(pin10) |> summarize(n = n()) |> filter(n > 1) |> count(pin10) > 0)
dups <- parcels_with_firm_dates |> group_by(pin10) |> mutate(n = n()) |> filter(n > 1)
dups |> group_by(pin10) |>  summarize(n = n_distinct(PRE_DATE)) |> filter(n > 1)  # do they have different dates though? Yes some do.
# DEAL WITH THESE LATER  ^^^^^ But Drop them for now
parcels_with_firm_dates <- parcels_with_firm_dates |> anti_join(dups)
...
nfhl_indicators <- nfhl_indicators |>
  mutate(
    across(bldg_sfha_2018:parcel_lomrs_2025,
      ~ replace_na(.x, FALSE)
  ))
...
# repeat_res_sales |> filter(pin10 %in% dups$pin10 & pin10 %in% indicator_combined$pin10)
...
single_res_sales |> filter(pin10 %in% indicator_combined$pin10[indicator_combined$parcel_sfha_2026 == TRUE])
single_res_sales |> filter(pin10 %in% indicator_combined$pin10[indicator_combined$parcel_sfha_2018 == TRUE])

sales_joined <- repeat_res_sales |>
  left_join(nfhl_indicators, by = "pin10")

sales_joined <- sales_joined |> fill_missing_panels()

sales_joined |> filter(is.na(PRE_DATE)) |> filter(!pin10 %in% drop_parcels)
missing_data <- sales_joined |> filter(!pin10 %in% drop_parcels) |> filter(is.na(FIRM_PAN)) |>
  distinct(pin,  pin10, sale_date, sale_price)

sales_joined |> filter(is.na(FIRM_PAN)) |> filter(!pin10 %in% drop_parcels)
missing_data |> group_by(pin10) |> summarize(n = n()) |> arrange(desc(n))
...
sales_joined |> filter(is.na(PRE_DATE))
...
sales_joined <- sales_joined |> anti_join(missing_data)
...
# sales_joined$sfha2018_col <- sales_joined$bldg_sfha_2018
...
sales_prepped3 <- sales_prepped3 |> select(-c(is_mydec_date, sale_filter_less_than_10k, row_id, sale_filter_deed_type, sale_filter_same_sale_within_365))
...
df_prep_final <- df_prep_final |> fill_missing_muni_by_nbhd_zip() |> final_df_prep_filters()
