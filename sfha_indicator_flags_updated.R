...
# ============================================================
# Township-by-township reports (HTML) for conflicting PINs
# ============================================================
# Save the sf object used for mapping/reporting so Quarto can read it.
# (This avoids re-running the whole targets load + joins inside each render.)
out_data_dir <- file.path("outputs", "township_reports", "data")
dir.create(out_data_dir, recursive = TRUE, showWarnings = FALSE)

saveRDS(sfha_conflict_sf, file.path(out_data_dir, "sfha_conflict_sf.rds"))

# Render one report per township (01–30)
library(quarto)
sfha_conflict_sf <- read_rds("outputs/township_reports/data/sfha_conflict_sf.rds")
out_report_dir <- file.path("outputs", "township_reports")
dir.create(out_data_dir, recursive = TRUE, showWarnings = FALSE)

conflict_index <- sfha_conflict_sf |>
  sf::st_drop_geometry() |>
  mutate(
    pin10 = as.character(pin10),
    township = stringr::str_sub(pin10, 1, 2)
  )

for (t in 1:30) {
  twp <- sprintf("%02d", t)

  n_twp <- conflict_index |>
    dplyr::filter(township == twp) |>
    nrow()

  if (n_twp == 0) next

  quarto::quarto_render(
    input = "township_conflict_report.qmd",
    output_file = paste0("township_", twp, "_conflict_report.html"),
    execute_params = list(
      township = twp,
      data_rds = file.path(out_data_dir, "sfha_conflict_sf.rds")
    )
  )
}
