...
table(bldg_sfha$eff_sfha_category)
table(bldg_sfha$change_type)
```
...
# Appendix - Nearby Control PINs only
...
prelim_twfe <- list(
 ...
 "I" =  feols(
    ...
    log(sale_price) ~ addedto_prelim_sfha  | pin + sale_year,
    cluster = ~ pin10,
        data = (df |> filter(change_type_prelim != "Always SFHA" & treat_year_remove_prelim == 10000 & nearby_600 == TRUE))),

  "II" =  feols(
    log(sale_price) ~ removedfrom_prelim_sfha | pin + sale_year,
    cluster = ~ pin10,
        data =(df |> filter(nearby_600 == TRUE))
    ),
  ...
  "II Subset" =  feols(
    log(sale_price) ~ removedfrom_prelim_sfha | pin + sale_year,
    cluster = ~ pin10,
        data = (df |> filter(change_type_prelim != "Never SFHA" & treat_year_add_prelim == 10000& nearby_600 == TRUE))
...
eff_twfe <- list(
 ...
 "I" =  feols(
    ...
    log(sale_price) ~ addedto_eff_sfha  | pin + sale_year,
    cluster = ~ pin10,
        data = (df |> filter(change_type != "Always SFHA" & treat_year_remove_eff == 10000 & nearby_600 == TRUE))),
  ...
  "II Subset" =  feols(
    log(sale_price) ~ removedfrom_eff_sfha | pin + sale_year,
    cluster = ~ pin10,
        data = (df |> filter(change_type != "Never SFHA" & treat_year_add_eff == 10000& nearby_600 == TRUE))
